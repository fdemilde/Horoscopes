...
@property (strong, nonatomic) NSMutableArray *buttonsText;
...
- (instancetype)initWithButtonFrame:(CGRect)centerButtonFrame
    ...
    if (self = [super init]) {
        ...
        _buttonsText = [[NSMutableArray alloc]init];
...
- (void)pathCenterButtonFold {
    ...
    for (int i = 0; i < self.itemButtons.count; i++) {
        ...
        UILabel *label = self.buttonsText[i];
        label.hidden = true;
...
//    rotationAnimation.duration = 0.35f;
    ...
    // BINH BINH: comment this out so all buttons will not gather to 1 place before going back animation
    // --------------------------------
...
- (void)pathCenterButtonBloom {
    ...
    for (int i = 0; i < self.itemButtons.count; i++) {
        ...
        UILabel *label = _buttonsText[i];
        label.hidden = false;
        label.textColor = [UIColor whiteColor];
        [label sizeToFit];
        label.frame = CGRectMake(pathItemButton.frame.origin.x - label.frame.size.width - 20, pathItemButton.frame.origin.y, label.frame.size.width, label.frame.size.height);
        ...
        [self insertSubview:label belowSubview:self.pathCenterButton];
        ...
        CGPoint textEndPoint = CGPointMake(endPoint.x - pathItemButton.frame.size.width/2 - 8 - label.frame.size.width/2, endPoint.y); // 20 is the padding b/w text and button
        label.center = textEndPoint;
...
- (void)addButtonText:(NSArray *)textArray {
    for(NSString *string in textArray){
        ...
        label.text = string;
        [self.buttonsText addObject:label];
    }
}
...
- (void)itemButtonTapped:(DCPathItemButton *)itemButton {
    for(UILabel * label in _buttonsText){
        label.hidden = true;
    }
