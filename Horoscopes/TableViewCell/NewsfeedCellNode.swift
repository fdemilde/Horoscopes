...
class NewsfeedCellNode : ASCellNode {
    ...
    func createFeedHeader(){
        ...
        let nameDict = [NSForegroundColorAttributeName: UIColor.blackColor(), NSFontAttributeName : UIFont.boldSystemFontOfSize(13.0)]
        userNameLabelNode?.attributedString = NSAttributedString(string: userPost!.user!.name, attributes: nameDict)
        println("userPost!.user!.name userPost!.user!.name = \(userPost!.user!.name)")
        background!.addSubnode(userNameLabelNode)
        
        feedTypeLabelNode = ASTextNode()
        let typeDict = [NSForegroundColorAttributeName: UIColor.blackColor(), NSFontAttributeName : UIFont.systemFontOfSize(11.0)]
        feedTypeLabelNode?.attributedString = NSAttributedString(string: self.getFeedTypeText(), attributes: typeDict)
        background!.addSubnode(feedTypeLabelNode)
    ...
    func createButtons(){
        heartImageView = ASImageNode()
        heartImageView?.backgroundColor = UIColor.clearColor()
        heartImageView?.image = UIImage(named: "newsfeed_heart_icon")
        background!.addSubnode(heartImageView)
    ...
    override func calculateSizeThatFits(constrainedSize: CGSize) -> CGSize {
        ...
        userNameLabelNode?.measure(constrainedSize)
        feedTypeLabelNode?.measure(constrainedSize)
        timePassedLabelNode?.measure(constrainedSize)
        feedDescriptionLabelNode?.measure(constrainedSize)
    ...
    override func layout(){
        ...
        self.feedTypeLabelNode!.frame = CGRectMake(self.userNameLabelNode!.frame.origin.x + userNameLabelNode!.calculatedSize.width + 5, 13, feedTypeLabelNode!.calculatedSize.width, feedTypeLabelNode!.calculatedSize.height)
        ...
        println("layout layout = \(NSStringFromCGSize(self.calculatedSize))")
        ...
        let sendAHeartButtonSize = CGSizeMake(80, 25)
        self.sendAHeartButton?.frame = CGRectMake(self.heartImageView!.frame.origin.x + heartImageSizeValue, self.background!.frame.height - sendAHeartButtonSize.height, sendAHeartButtonSize.width, sendAHeartButtonSize.height)
        
        self.shareButton?.frame = CGRectMake(self.background!.frame.width - self.shareButton!.frame.width - 15, self.shareButton!.frame.width, sendAHeartButtonSize.width, self.shareButton!.frame.height)
